package edu.purdue.cs59000_12.undergradExample;
...
//    UMLCreateMessage createPizzaOrder =
//        new UMLCreateMessage("pizzaOrder", pizzaOrder, null, customerDining, umlSD);
//    UMLLifeline pizzaOrderLifeline = new UMLLifeline(pizzaOrder, umlSD);
//    createPizzaOrder.setDestination(
//        pizzaOrder); // destinations of create messages should be UMLClasses
    //
        UMLActivationBox chefCooking = new UMLActivationBox(chefLifeline, umlSD);
    //
        UMLMessageArgument pizzaOrderArg =
            new UMLMessageArgument("pizzaOrder", "PizzaOrder", "pizzaOrder");
        ArrayList<UMLMessageArgument> pizzaOrderArgs = new ArrayList<UMLMessageArgument>();
        pizzaOrderArgs.add(pizzaOrderArg);
    UMLSynchronousMessage requestPizza =
        new UMLSynchronousMessage(
            "foodOrder", "Pizza", pizzaOrderArgs, customerDining, chefCooking, umlSD);
...
//        UMLCreateMessage createPizzaDough =
//            new UMLCreateMessage("pizzaDough", pizzaDough, null, chefPreparingDough, umlSD);
    ...
    //    // knead the dough until it's ready
        ...
        UMLLoop doughKneadLoop =
            new UMLLoop(
                doughReady,
                umlSD); // automatically sets doughReady's parent container as doughKneadLoop
    //
    //    // kneading the dough
        UMLMessageArgument kneadDoughArg = new UMLMessageArgument("pizzaDough", "Dough",
     "pizzaDough");
        ArrayList<UMLMessageArgument> kneadDoughArgs = new ArrayList<UMLMessageArgument>();
        kneadDoughArgs.add(kneadDoughArg);
        UMLSynchronousMessage kneadDough =
            new UMLSynchronousMessage(
                "kneadDough", "boolean", kneadDoughArgs, chefPreparingDough, doughPreparing,
     umlSD);
    kneadDough.setParentContainer(
        doughReady); // need to contain messages with containers if inside conditionals
    //
        UMLReturnMessage returnDoughReady =
            new UMLReturnMessage("doughReady", doughPreparing, chefPreparingDough, umlSD);
        returnDoughReady.setParentContainer(
            doughReady); // need to contain messages with containers if inside conditionals
    //
...
//        UMLCreateMessage addToppings =
//            new UMLCreateMessage("toppedDough", toppedDough, toppings, toppedDough, umlSD);
    ...
    //    // determine the cheese type
        ...
        UMLElse normalCheese =
            new UMLElse(
                umlSD); // in the diagram, this is specified as "NormalCheese", which should just
    // be
    //    // else
        UMLAlternatives cheeseSelection =
            new UMLAlternatives(crispyCheese, normalCheese, umlSD); // automatically sets
    // condition and
...
//    UMLSynchronousMessage crispyCheeseMsg =
//        new UMLSynchronousMessage("cook", "Pizza", null, chefCooking, ovenBaking, umlSD);
//    crispyCheeseMsg.addArgument(
//        new UMLMessageArgument("toppedDough", "ToppedDough", "toppedDough"));
//    crispyCheeseMsg.addArgument(new UMLMessageArgument("cookTime", "int", "30"));
//    crispyCheeseMsg.setParentContainer(
//        crispyCheese); // need to contain messages with containers if inside conditionals
    //
//    //    // set the oven to bake for 20 minutes to make normal cheese
//    UMLSynchronousMessage normalCheeseMsg =
//        new UMLSynchronousMessage("cook", "Pizza", null, chefCooking, ovenBaking, umlSD);
//    normalCheeseMsg.addArgument(
//        new UMLMessageArgument("toppedDough", "ToppedDough", "toppedDough"));
//    normalCheeseMsg.addArgument(new UMLMessageArgument("cookTime", "int", "20"));
//    normalCheeseMsg.setParentContainer(
//        normalCheese); // need to contain messages with containers if inside conditionals
    //
...
//    UMLSynchronousMessage createPizza =
//        new UMLSynchronousMessage(
//            "createPizza", "Pizza", null, ovenBaking, createPizzaActivation, umlSD);
...
//        UMLCreateMessage createCookedPizza =
//            new UMLCreateMessage("cookedPizza", cookedPizza, null, createPizzaActivation, umlSD);

        UMLLifeline cookedPizzaLifeline = new UMLLifeline(cookedPizza, umlSD);
    //
    //    // oven completes baking the pizza
        UMLReturnMessage returnCookedPizzaToOven =
            new UMLReturnMessage("cookedPizza", createPizzaActivation, ovenBaking, umlSD);
    //
    //    // chef removes the pizza from the oven
        UMLReturnMessage returnCookedPizzaToChef =
            new UMLReturnMessage("cookedPizza", ovenBaking, chefCooking, umlSD);
    //
...
//    UMLSynchronousMessage coolPizza =
//        new UMLSynchronousMessage("cool", "Pizza", null, chefCooking, coolPizzaActivation, umlSD);
//    coolPizza.addArgument(new UMLMessageArgument("cookedPizza", "Pizza", "cookedPizza"));
    ...
    //
    //    // cooling cycle
        UMLSynchronousMessage coolCycle =
            new UMLSynchronousMessage(
                "coolCycle", "boolean", null, coolPizzaActivation, coolPizzaActivation, umlSD);
        coolCycle.addArgument(new UMLMessageArgument("cookedPizza", "Pizza", "cookedPizza"));
        coolCycle.setParentContainer(
            cooled); // need to contain messages with containers if inside conditionals
    //
    //    // return cooling status
        UMLReturnMessage coolPizzaStatus =
            new UMLReturnMessage("cooled", coolPizzaActivation, coolPizzaActivation, umlSD);
        coolPizzaStatus.setParentContainer(
            cooled); // need to contain messages with containers if inside conditionals

    //    // if pizza is cooled, then return finished pizza to chef
        UMLReturnMessage pizzaCooled =
            new UMLReturnMessage("finishedPizza", coolPizzaActivation, chefCooking, umlSD);
    //
    //    // deliver the finished pizza to the customer
        UMLReturnMessage deliverPizza =
            new UMLReturnMessage("finishedPizza", chefCooking, customerDining, umlSD);
    //  }
