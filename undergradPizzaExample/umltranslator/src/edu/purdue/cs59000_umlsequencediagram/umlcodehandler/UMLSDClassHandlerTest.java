...
public class UMLSDClassHandlerTest {
  ...
  public void generateSourceCodeTest() throws Exception {
    ...
    UMLClass.setCodeHandler(
        new UMLSDClassHandler()); // setting the source code handler to UMLSDClassHandler
    ...
    UMLClass class2 =
        new UMLClass("Class2", "class2", umlSD); // creating a second class and it's children
    ...
    UMLClass class3 =
        new UMLClass(
            "Class2", "class3",
            umlSD); // creating another instance of Class2, this shouldn't affect compUnits
    UMLLifeline lifeline3 = new UMLLifeline(class3, umlSD);
    UMLActivationBox actBox3 = new UMLActivationBox(lifeline3, umlSD);

    UMLSynchronousMessage message =
        new UMLSynchronousMessage(
            "method1", "String", new ArrayList<UMLMessageArgument>(), actBox1, actBox2, umlSD);

    LinkedList<CompilationUnit> compUnits =
        new LinkedList<CompilationUnit>(); // adding an empty compilation unit list to be filled
    try {
    ...
    for (CompilationUnit compUnit :
        compUnits) { // checking if classes exist in compUnits and assigning to Optionals if they do
